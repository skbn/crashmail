...
#include <proto/dos.h>
/* AmigaOS Delay() uses ticks (50 per second), so multiply seconds by 50 */
#define JAM_Sleep(x) Delay((x)*50)
#endif
...
int jam_Lock( s_JamBase* Base_PS, int DoLock_I )
{
    ...
    return 0;
#elif defined(__AMIGA__) || defined(PLATFORM_AMIGA)
	/* AmigaOS 3.x doesn't have fcntl-style file locking */
