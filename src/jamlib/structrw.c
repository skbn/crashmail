...
/* sign extension fix */
uint16_t jamgetuword(const char *buf, uint32_t offset)
{
    return ((uint16_t)(uint8_t)buf[offset]) | ((uint16_t)(uint8_t)buf[offset + 1] << 8);
}

void jamputuword(char *buf, uint32_t offset, uint16_t num)
{
    buf[offset] = (char)(num & 0xFF);
    buf[offset + 1] = (char)((num >> 8) & 0xFF);
}

uint32_t jamgetuint32_t(const char *buf, uint32_t offset)
{
    return ((uint32_t)(uint8_t)buf[offset]) | ((uint32_t)(uint8_t)buf[offset + 1] << 8) | ((uint32_t)(uint8_t)buf[offset + 2] << 16) | ((uint32_t)(uint8_t)buf[offset + 3] << 24);
}
...
void jamputuint32_t(char *buf, uint32_t offset, uint32_t num)
{
    buf[offset] = (char)(num & 0xFF);
    buf[offset + 1] = (char)((num >> 8) & 0xFF);
    buf[offset + 2] = (char)((num >> 16) & 0xFF);
    buf[offset + 3] = (char)((num >> 24) & 0xFF);
}
