...
void jam_gethighwater(struct jam_Area *ja)
{
   ...
   {
      /* Read as bytes and convert from little-endian (endian-safe) */
      if(osRead(fh,bytebuf,4) == 4)
      {
         num = (uint32_t)(bytebuf[0] + 
                         bytebuf[1]*256 + 
                         bytebuf[2]*256*256 + 
                         bytebuf[3]*256*256*256);
         ja->HighWater=num;
         ja->OldHighWater=num;
      }
...
void jam_writehighwater(struct jam_Area *ja)
{
   ...
   {
      /* Write as bytes in little-endian format (endian-safe) */
      bytebuf[0] = num % 256;
      bytebuf[1] = (num / 256) % 256;
      bytebuf[2] = (num / 256 / 256) % 256;
      bytebuf[3] = (num / 256 / 256 / 256) % 256;
      osWrite(fh,bytebuf,4);
      osClose(fh);
   }
}
