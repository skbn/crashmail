...
bool IsArc(char *file)
{
   ...
   if(strlen(file)<11) return(FALSE);
   if(file[8]!='.')    return(FALSE);

   for(c=0;c<8;c++)
      if(!isalnum((unsigned char)file[c])) return(FALSE);

   strncpy(ext,&file[9],2);
   ext[2]=0;

   if(stricmp(ext,"MO")!=0 && stricmp(ext,"TU")!=0 && stricmp(ext,"WE")!=0 &&
      stricmp(ext,"TH")!=0 && stricmp(ext,"FR")!=0 && stricmp(ext,"SA")!=0 &&
      stricmp(ext,"SU")!=0) return(FALSE);

   /* Validate optional suffix after the two-letter day: [0-9]*(\.[0-9]+)? */
   p = &file[11];
...
bool IsPkt(char *file)
{
   ...
   /* Check that chars 9-11 are 'pkt' (case-insensitive) */
   if(tolower(file[9])!='p' || tolower(file[10])!='k' || tolower(file[11])!='t')
      return(FALSE);

   /* Validate optional suffix after pkt: [0-9]*(\.[0-9]+)? */
   p = &file[12];
