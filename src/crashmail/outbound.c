...
void MakeBaseName(struct Node4D *n4d,char *basename)
{
   ...
   use_aso = (config.cfg_Flags & CFG_ASO) ? TRUE : FALSE;
   ...
   if(use_aso)
   {
      /* ASO - Amiga Style Outbound (flat files with zone.net.node.point naming) */
      /* Format: outbound/ZONE.NET.NODE.POINT (decimal, no padding) */
      ...
      if(basename[0])
      {
         if(strchr(ospathchars,basename[strlen(basename)-1]))
            basename[strlen(basename)-1]=0; /* Strip path separator */
      }
      ...
      /* Add zone.net.node.point (decimal, no padding) */
      sprintf(buf,"%u.%u.%u.%u", n4d->Zone, n4d->Net, n4d->Node, n4d->Point);
      strcat(basename,buf);
   }
   else
   {
      /* BSO - Binkley Style Outbound (5D structure) */
