...
#ifdef PLATFORM_AMIGA
/* Check if a task pointer is still present in Exec task lists */
static bool AmigaTaskAlive(void *taskptr)
{
   struct Task *t;

   Forbid();
   for(t=(struct Task *)SysBase->TaskReady.lh_Head;
       t->tc_Node.ln_Succ!=NULL;
       t=(struct Task *)t->tc_Node.ln_Succ)
   {
      if(t==(struct Task *)taskptr)
      {
         Permit();
         return(TRUE);
      }
...
bool LockConfig(char *file)
{
	...
	{
#ifdef PLATFORM_AMIGA
		/* Read task pointer written by the owner; remove lock if owner is gone */
		void *taskptr=NULL;
		FILE *bfp=fopen(buf,"r");

		if(bfp)
		{
			fscanf(bfp,"%p",&taskptr);
			fclose(bfp);
		}

		if(taskptr && !AmigaTaskAlive(taskptr))
		{
			printf("Configuration file %s: owner task gone, removing stale lock\n",file);
			osDelete(buf);
			continue;
		}
#endif
		if(++retries > 6)
		{
			printf("Configuration file %s: timed out waiting for lock\n",file);
			return(FALSE);
		}
...
#ifdef PLATFORM_AMIGA
	/* Write current task pointer so future instances can detect a stale lock */
	fprintf((FILE *)fp,"%p\n",(void *)FindTask(NULL));
#endif
