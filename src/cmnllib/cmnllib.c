...
uint16_t cmnlgetuword(uint8_t *buf,uint32_t offset)
{
   ...
   /* Amiga: Alignment-safe little-endian read */
   return ((uint16_t)(uint8_t)buf[offset]) | ((uint16_t)(uint8_t)buf[offset+1] << 8);
#else
   /* Linux/Windows: Original implementation */
...
{
#ifdef PLATFORM_AMIGA
   /* Amiga: Alignment-safe little-endian read of 4 bytes */
...
#else
   /* Linux/Windows: Original implementation */
   return (long) buf[offset]+buf[offset+1]*256+buf[offset+2]*256*256+buf[offset+3]*256*256*256;
#endif
}
