...
int ed_undo_stack_make_room(UndoGroup **stack, int *top, int *cap, int max)
{
    ...
    /* Pathological: caller configured zero undo depth. Refuse to grow
     * the stack rather than entering the drop-oldest path with no
     * entries to drop (the memmove below would interpret -1 as a huge
     * unsigned and crash) */
    if (max <= 0)
        return -1;
