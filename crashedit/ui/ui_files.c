...
#define UI_FILES_MAX_ENTRIES 16384
...
static void wchar_to_char(const wchar_t *src, char *dst, int dst_sz)
{
    ...
    for (i = 0, j = 0; src[i] && j < dst_sz - 1; i++)
    {
        ...
        else if (w < 0x800)
        {
            if (j + 1 < dst_sz - 1)
            {
                dst[j++] = (char)(0xC0 | ((w >> 6) & 0x1F));
                dst[j++] = (char)(0x80 | (w & 0x3F));
            }
        }
        else if (w < 0x10000)
        {
            if (j + 2 < dst_sz - 1)
            {
                dst[j++] = (char)(0xE0 | ((w >> 12) & 0x0F));
                dst[j++] = (char)(0x80 | ((w >> 6) & 0x3F));
                dst[j++] = (char)(0x80 | (w & 0x3F));
            }
        }
        else
        {
            if (j + 3 < dst_sz - 1)
            {
                dst[j++] = (char)(0xF0 | ((w >> 18) & 0x07));
                dst[j++] = (char)(0x80 | ((w >> 12) & 0x3F));
                dst[j++] = (char)(0x80 | ((w >> 6) & 0x3F));
                dst[j++] = (char)(0x80 | (w & 0x3F));
            }
        }
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            for (i = 0; i < visible && top + i < nents; i++)
            {
                ...
                attrset(COLOR_PAIR(idx == sel && !edit_dir ? COL_POPUP_SEL : COL_POPUP));
...
int ui_files_save(const char *title, const char *start_dir, const char *init_name, char *out_path, int out_path_sz)
{
    ...
    for (;;)
    {
        ...
        for (;;)
        {
            ...
            input_draw(&dir_state, y + 2, x + 8, w - 10, edit_dir);

            /* Name line - highlight label too when active */
            attrset(COLOR_PAIR(edit_name ? COL_POPUP_SEL : COL_POPUP));
            mvaddnstr(y + 3, x + 2, "Name: ", 6);
            standend();

            input_draw(&name_state, y + 3, x + 8, w - 10, edit_name);
            ...
            for (i = 0; i < visible && top + i < nents; i++)
            {
                ...
                attrset(COLOR_PAIR(idx == sel && !edit_dir && !edit_name ? COL_POPUP_SEL : COL_POPUP));
            ...
            if (edit_name)
            {
                if (key == '\n' || key == '\r' || key == KEY_ENTER)
                {
                    ...
                    {
                        char tmp[UI_FILES_PATH_MAX];
                        wchar_to_char(dir_w, tmp, sizeof(tmp));
                        wchar_to_char(name_w, name_input, sizeof(name_input));
            ...
            else if (key == '\n' || key == '\r' || key == KEY_ENTER)
            {
                ...
                else
                {
                    ...
                    char_to_wchar(ents[sel].name, name_w, sizeof(name_w) / sizeof(wchar_t));
