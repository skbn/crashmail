...
/* Bridge: gramcheck asks is this word correct -> host Hunspell, ud is UiApp*, we read spell_handle each call so dictionary swap at runtime is picked up automatically, word arrives as non-NUL-terminated span so copy to small stack buffer */
static int gg_spell_bridge(void *ud, const char *utf8_word, int len)
{
    UiApp *app = (UiApp *)ud;
    char buf[96];

    if (!app || !app->spell_handle || !app->spell_active)
        return 1; /* no dictionary -> treat as correct (rule inert) */

    if (len <= 0 || (size_t)len >= sizeof(buf))
        return 1;

    memcpy(buf, utf8_word, (size_t)len);

    buf[len] = '\0';

    return spell_check((SpellChecker *)app->spell_handle, buf);
}
#endif /* HAVE_HUNSPELL */
...
int ui_grammar_load_from_config(UiApp *app)
{
    char path[512];
    GramCheck *g = NULL;
...
#ifdef HAVE_HUNSPELL
    /* Wire category 1 (Ortografía): grammar's SPELL_UNKNOWN rule will consult the editor's Hunspell dictionary through this bridge */
    gc_set_spell_callback(g, gg_spell_bridge, (void *)app);
#endif
...
void ui_grammar_free_app(UiApp *app)
{
...
void ui_grammar_prewarm(UiApp *app, int top, int body_rows, int line_count)
{
    Ed *ed = NULL;
...
void ui_grammar_invalidate_row(UiApp *app, const wchar_t *wl, int line_len)
{
    char utf8[UI_GRAM_MAX_UTF8];
    int u;
...
void ui_grammar_flush(UiApp *app)
{
    if (!app || !app->grammar_handle)
        return;
...
void ui_grammar_apply_toggles(UiApp *app)
{
    GramCheck *g = NULL;
    unsigned mask;
