...
/* TTS glue for crashedit, supports editor and reader surfaces */
...
/* wchar_t to UTF-8 conversion, shared with other modules */
...
void ui_tts_load_from_config(UiApp *app)
{
    ...
    else
    {
        ...
        {
            app->tts_enabled = 0;
            ui_status(app, "TTS backend unavailable (espeak-ng not found?)");
        }
        else
        {
            ui_status(app, "TTS enabled");
        }
    }
}
...
/* Text extraction from editor */
static char *grab_editor_text(UiApp *app)
{
    Ed *ed = NULL;
    EdInfo info;
    int first;
...
/* Text extraction from reader */
static char *grab_reader_text(UiApp *app)
{
    Reader *rd = NULL;
    int total;
    wchar_t *joined = NULL;
    ...
    for (i = 0; i < total; i++)
    {
        const wchar_t *line = rd_get_line(rd, i);
        int ll;
...
/* Grab text from active surface (reader or editor) */
static char *grab_current_text(UiApp *app)
{
    if (!app)
        return NULL;

    /* Prefer reader if showing message, only non-NULL when actively reading */
    if (app->reader)
        return grab_reader_text(app);

    if (app->editor)
        return grab_editor_text(app);
...
void ui_tts_speak_action(UiApp *app)
{
    ...
    {
        ui_status(app, "TTS not available");
        return;
    }

    text = grab_current_text(app);

    if (!text || !text[0])
    {
        free(text);

        ui_status(app, "Nothing to speak");
        return;
    }

    tts_speak((TtsHandle *)app->tts_handle, text);

    ui_status(app, "Speaking...");
...
{
    /* Both editor and reader speak whole message, no cursor-to-end in reader */
    ui_tts_speak_action(app);
}

/* Grab all editor buffer rows from line 0 */
static char *grab_editor_full(UiApp *app)
{
    Ed *ed = NULL;
    EdInfo info;
    wchar_t *joined = NULL;
    ...
    for (row = 0; row < info.line_count; row++)
    {
        const wchar_t *line = ed_line_wcs(ed, row);
        int ll = ed_line_len(ed, row);
...
/* Dictate whole message from top, restarts speech, bound to Alt+Shift+K */
void ui_tts_speak_doc_action(UiApp *app)
{
    ...
    if (app->reader)
        text = grab_reader_text(app);
    else
        text = grab_editor_full(app);
    ...
    ui_status(app, "Speaking whole message...");
...
void ui_tts_pause_toggle(UiApp *app)
{
    ...
    if (st == TTS_STATE_PLAYING)
    {
        ...
        ui_status(app, "Speech paused");
    }
    else if (st == TTS_STATE_PAUSED)
    {
        tts_resume((TtsHandle *)app->tts_handle);
        ui_status(app, "Speech resumed");
    }
    else
    {
        ui_status(app, "Not speaking");
    }
}
...
void ui_tts_popup(UiApp *app)
{
    ...
    if (result_persist < 0)
    {
        ...
        ui_status(app, "Speech settings unchanged");
        return;
    }
    ...
    if (result_persist == 1)
    {
        cfg_save(app->cfg, app->cfg_path);
        ui_status(app, "Speech settings saved");
    }
    else
    {
        ui_status(app, "Speech settings applied (session only)");
    }
}
...
int ui_tts_is_active(void *vapp)
{
    ...
    return tts_get_state((TtsHandle *)app->tts_handle) != TTS_STATE_IDLE;
}
