...
int ui_dict_picker(UiApp *app)
{
    ...
    if (!result)
    {
        ...
        if (n_sugg <= 0)
        {
            ui_status(app, "No translation for '%s'", word_buf);
        ...
        if (!result)
        {
            ui_status(app, "Lookup of '%s' failed: %s", word_buf, err[0] ? err : "(unknown)");
    ...
    if (n_items == 0)
    {
        ui_status(app, "No usable translation for '%s'", word_buf);
