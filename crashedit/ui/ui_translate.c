...
int ui_translate_load_from_config(UiApp *app)
{
    ...
    {
        ui_status(app, "Translator: cannot init network");
        return 0;
    }
    ...
    if (!h)
    {
        ...
        ui_status(app, "Translator: handle init failed");
        return 0;
    }
...
static int replace_lines(UiApp *app, int first, int last, const char *new_utf8)
{
    Ed *ed = app->editor;
...
static int translate_show_popup(UiApp *app, const char *src, const char *dst, const char *result_utf8, const char *err_msg)
{
    ...
    {
        ui_status(app, "Translate error: %s", err_msg);
        return 0;
    }

    if (!result_utf8 || !result_utf8[0])
    {
        ui_status(app, "Translate: empty result");
        return 0;
    }

    /* Show the result in the status bar (compact) plus offer actions */
    ui_status(app, "Translated: %.120s", result_utf8);
    ...
    if (sel == 2)
    {
        ...
        if (clipboard_copy(result_utf8) == 0)
            ui_status(app, "Translation copied to clipboard");
        else
            ui_status(app, "Clipboard copy failed");
#else
        ui_status(app, "Clipboard not available; translation kept in cache");
#endif
...
int ui_translate_action(UiApp *app)
{
    ...
    {
        ui_status(app, "Translator disabled (press Ctrl+T to enable)");
        return 1;
    }
    ...
    {
        ui_status(app, "No text to translate");
    ...
    ui_status(app, "Translating %s -> %s...", from_lang, to_lang);
    refresh();
    ...
    if (!result)
    {
        ui_status(app, "Translate failed: %s", err[0] ? err : "(unknown)");
        free(src);
        return 1;
    }
    ...
    else if (choice == 2)
    {
        ...
        if (!ed)
        {
            ui_status(app, "Insert failed: no editor");
        }
        else
        {
...
{
    if (app)
        ui_status(app, "Translator not built in (USE_TRANSLATE=1)");
