...
/* Like binary_search but instead of returning -1 on miss, returns the
 * position where `word` would be inserted to keep the array sorted.
 * Used by translate_stardict_suggest to grab the entries that come
 * just before and just after the (non-existent) target */
 ...
 * case-insensitive over ASCII (matching sd_strcmp) */
