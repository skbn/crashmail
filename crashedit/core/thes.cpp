...
/* Cached lookup: pre-formatted synonym/def pairs to reduce allocations */
typedef struct
{
...
typedef struct
{
    char word[THES_CACHE_KEY_MAX];                      /* '\0' = empty slot */
    int nmeanings;                                      /* 0 = negative cache */
    int nsyns;                                          /* total synonyms */
    char defs[THES_CACHE_SYNS_MAX][THES_CACHE_TXT_MAX]; /* one def per meaning */
...
extern "C"
{
...
#ifdef HAVE_MYTHES
        MyThes *mt;
        SpellChecker *sc; /* borrowed speller */
        short cache_head;
        short cache_tail;
        short cache_count;
    ...
    /* Reconstruct ThesMeaning array from cache entry */
    ...
    static void tcache_store(ThesHandle *t, const char *word, const ThesMeaning *m, int nmeanings)
    {
        ...
        /* Cap meanings at THES_CACHE_SYNS_MAX */
    ...
    /* Negative cache: mark word as no results */
    ...
    ThesHandle *thes_new(const char *idx_path, const char *dat_path)
    {
        ...
        /* Probe files first to avoid broken MyThes object */
    ...
    void thes_free(ThesHandle *t)
    {
        ...
        /* Cache lives inline, one free() reclaims all */
        free(t);
    }
    ...
    /* Append add onto acc, growing as needed. Returns 1 ok, 0 on OOM */
    ...
    int thes_lookup(ThesHandle *t, const char *word, ThesMeaning **out_meanings)
    {
        ...
        if (cache_idx != -1)
        {
            ...
            /* Negative cache: word has no synonyms */
            if (e->nmeanings == 0 || e->nsyns == 0)
                return 0;
        ...
        /* Try Hunspell stems on verbatim miss */
        if (t->sc)
        {
            char **stems = NULL;
            ...
            for (i = 0; i < ns; i++)
            {
                ...
                {
                    /* nm == 0 but allocated -- shouldn't happen */
